...
const indexProto = {
    toString: function (): string {
        return _(this)
            .toPairs()
            .map(([k, v]) => k ? `[${_.upperFirst(_.lowerCase(k as string))}] ${v}` : '')
            .value()
            .join('\n') + '\n';
    }
};
...
export class CrawlerHost extends RPCHost {
    ...
    private sendFormattedResponse(res: Response, formatted: any, respondWith: string) {
        if (respondWith === 'screenshot' && Reflect.get(formatted, 'screenshotUrl')) {
            return sendResponse(res, `${formatted}`,
                { code: 302, headers: { Location: Reflect.get(formatted, 'screenshotUrl') } }
            );
        }
        if (respondWith === 'pageshot' && Reflect.get(formatted, 'pageshotUrl')) {
            return sendResponse(res, `${formatted}`,
                { code: 302, headers: { Location: Reflect.get(formatted, 'pageshotUrl') } }
            );
        }
    ...
    configure(opts: CrawlerOptions, req: Request, urlToCrawl: URL) {
        ...
        const crawlOpts: ExtraScrappingOptions = {
            ...
            cookies: cookies,
            favorScreenshot: ['screenshot', 'pageshot'].includes(opts.respondWith || 'markdown'),
